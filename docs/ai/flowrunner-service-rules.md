...
- **No `order` property** - Do NOT add an `order` property to config items. Display order is dictated by the position of each item in the array passed to `addService()`, so `order` is redundant — never add it, and remove it from any legacy config items.
